...
export function App() {
  ...
  const defaultOutboundInputId = inputDevices[0]?.deviceId || ''
  const virtualCableOutput = outputDevices.find(isVirtualCableDevice)
  const defaultOutboundOutputId = virtualCableOutput?.deviceId || outputDevices[0]?.deviceId || ''

  const virtualCableInput = inputDevices.find(isVirtualCableDevice)
  ...
  const defaultInboundOutputId = headphonesOutput?.deviceId || outputDevices.find(d => d.deviceId !== defaultOutboundOutputId)?.deviceId || outputDevices[0]?.deviceId || ''
  ...
  return (
    <main className="app-shell">
      ...
      <section className="actions" aria-label="Acoes globais" style={{ marginBottom: '2rem' }}>
        <button
          type="button"
      ...
      <div style={{ display: 'grid', gap: '2rem' }}>
        <TranslationSession
          ...
          outputLabel="Saída para o Meet (VB-Cable)"
        ...
        <TranslationSession
          ...
          inputLabel="Áudio do Meet (Loopback ou Cable)"
      ...
      <section className="runtime-strip" aria-label="Ambiente" style={{ marginTop: '2rem' }}>
        <div>
          <span>API local</span>
